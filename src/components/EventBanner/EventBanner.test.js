...
describe("EventBanner appearance", () => {
    ...
    it("should have a meeting link if passed in", () => {
        let wrapper = mount(<EventBanner flyerSource="Test.jpg" meetingLink="https://example.com"/>);
    ...
    it("should indicate if the event is in progress", () => {
        ...
        let wrapper = mount(<EventBanner title="Title" startDate={dayjs("2020-04-04T03:00:00.000Z")} endDate={dayjs("2020-04-04T04:00:00.000Z")} now={now}/>, container)
...
describe("Event auto expire", () => {
    ...
    it("should not render if it's a GBM, and the start time is more than 7 days in the future", () => {
        ...
        let wrapper = mount(<EventBanner title="GBM" isGbm={true} startDate={dayjs("2020-04-09T21:00:00.000Z")} endDate={dayjs("2020-04-09T22:00:00.000Z")} now={now}/>, container);
        expect(wrapper.exists(".flyer")).toEqual(false);

        now = dayjs("2020-03-01T16:00:00.000Z");
        wrapper = mount(<EventBanner title="GBM" isGbm={true} startDate={dayjs("2020-03-09T21:00:00.000Z")} endDate={dayjs("2020-03-09T22:00:00.000Z")} now={now}/>, container);
        expect(wrapper.exists(".flyer")).toEqual(false);

        now = dayjs("2020-04-04T21:15:00.000Z").subtract(7, 'day');
        wrapper = mount(<EventBanner title="GBM" isGbm={true} startDate={dayjs("2020-04-04T21:00:00.000Z")} endDate={dayjs("2020-04-04T22:00:00.000Z")} now={now}/>, container);
        expect(wrapper.exists(".flyer")).toEqual(true);
